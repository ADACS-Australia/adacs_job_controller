...
if clang-format-23 --dry-run --Werror $FILES >/dev/null 2>&1; then
    echo "✅ All code is properly formatted with clang-format!"
    exit 0
else
    echo "❌ Code is not properly formatted with clang-format!"
    echo "The following files need formatting:"
    clang-format-23 --dry-run $FILES 2>&1 | grep -E "(^src/|would be reformatted)" | grep -v "clang-format:"
    echo ""
    echo "Run the following command to fix formatting:"
    echo "find src -name \"*.cpp\" -o -name \"*.cppm\" -o -name \"*.ixx\" -o -name \"*.h\" | grep -v \"build/\" | grep -v \"third_party/\" | xargs clang-format-23 -i"
    exit 1
fi