...
BOOST_AUTO_TEST_CASE(test_mClusterPing_has_correct_entry_after_connection)
{
    ...
    BOOST_CHECK_EQUAL(
        std::static_pointer_cast<ClusterManager>(clusterManager)->getmClusterPings()->begin()->first,
        *std::static_pointer_cast<ClusterManager>(clusterManager)->getvClusters()->at(0)->getpConnection());
    BOOST_CHECK_MESSAGE(
        std::static_pointer_cast<ClusterManager>(clusterManager)->getmClusterPings()->begin()->second.pingTimestamp ==
            zeroTime,
        "pingTimestamp was not zero when it should have been");
    BOOST_CHECK_MESSAGE(
        std::static_pointer_cast<ClusterManager>(clusterManager)->getmClusterPings()->begin()->second.pongTimestamp ==
            zeroTime,
...
BOOST_AUTO_TEST_CASE(test_checkPings_send_ping_success)
{
    ...
    auto previousPingTimestamp =
        std::static_pointer_cast<ClusterManager>(clusterManager)->getmClusterPings()->begin()->second.pingTimestamp;
    auto previousPongTimestamp =
        std::static_pointer_cast<ClusterManager>(clusterManager)->getmClusterPings()->begin()->second.pongTimestamp;
    ...
    BOOST_CHECK_MESSAGE(
        std::static_pointer_cast<ClusterManager>(clusterManager)->getmClusterPings()->begin()->second.pingTimestamp >
            previousPingTimestamp,
        "pingTimestamp was not greater than the previous ping timestamp when it should not have been");
    BOOST_CHECK_MESSAGE(
        std::static_pointer_cast<ClusterManager>(clusterManager)->getmClusterPings()->begin()->second.pongTimestamp >
            previousPongTimestamp,
        "pongTimestamp was not greater than the previous pong timestamp when it should not have been");
