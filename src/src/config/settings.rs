...
fn env_or_uint<T>(key: &str, default: T) -> T
where
    T: FromStr + Display,
    T::Err: Debug,
{
...
pub static FILE_DOWNLOAD_EXPIRY_TIME: LazyLock<u64> =
    LazyLock::new(|| env_or_uint("FILE_DOWNLOAD_EXPIRY_TIME", 86400));
...
pub static QUEUE_SOURCE_PRUNE_MILLISECONDS: LazyLock<u64> =
    LazyLock::new(|| env_or_uint("QUEUE_SOURCE_PRUNE_MILLISECONDS", 60000));
...
pub static CLUSTER_RESEND_MESSAGE_INTERVAL_MILLISECONDS: LazyLock<u64> =
    LazyLock::new(|| env_or_uint("CLUSTER_RESEND_MESSAGE_INTERVAL_MILLISECONDS", 60000));
...
pub static CLIENT_TIMEOUT_SECONDS: LazyLock<u64> =
    LazyLock::new(|| env_or_uint("CLIENT_TIMEOUT_SECONDS", 30));
...
pub static MESSAGE_INITIAL_VECTOR_SIZE: LazyLock<u32> =
    LazyLock::new(|| env_or_uint("MESSAGE_INITIAL_VECTOR_SIZE", 65536));
...
pub static CLUSTER_RECENT_STATE_JOB_IGNORE_SECONDS: LazyLock<u64> =
    LazyLock::new(|| env_or_uint("CLUSTER_RECENT_STATE_JOB_IGNORE_SECONDS", 60));
...
pub static CLUSTER_MANAGER_CLUSTER_RECONNECT_SECONDS: LazyLock<u64> =
    LazyLock::new(|| env_or_uint("CLUSTER_MANAGER_CLUSTER_RECONNECT_SECONDS", 60));
...
pub static CLUSTER_MANAGER_PING_INTERVAL_SECONDS: LazyLock<u64> =
    LazyLock::new(|| env_or_uint("CLUSTER_MANAGER_PING_INTERVAL_SECONDS", 10));
pub static CLUSTER_MANAGER_MAX_TOKEN_EXPIRY_SECONDS: LazyLock<u64> =
    LazyLock::new(|| env_or_uint("CLUSTER_MANAGER_MAX_TOKEN_EXPIRY_SECONDS", 600));

// HTTP settings
/// TCP port for the HTTP REST API (`HTTP_PORT`).
pub static HTTP_PORT: LazyLock<u16> = LazyLock::new(|| env_or_uint("HTTP_PORT", 8000));
...
pub static RATE_LIMIT_REQUESTS_PER_SECOND: LazyLock<u64> =
    LazyLock::new(|| env_or_uint("RATE_LIMIT_REQUESTS_PER_SECOND", 0));
/// Burst allowance above the sustained rate (`RATE_LIMIT_BURST_SIZE`).
pub static RATE_LIMIT_BURST_SIZE: LazyLock<u32> =
    LazyLock::new(|| env_or_uint("RATE_LIMIT_BURST_SIZE", 50));
// WebSocket settings
/// TCP port for cluster WebSocket connections (`WEBSOCKET_PORT`).
pub static WEBSOCKET_PORT: LazyLock<u16> = LazyLock::new(|| env_or_uint("WEBSOCKET_PORT", 8001));
...
mod tests {
    ...
    fn test_env_or_helpers() {
        ...
        assert_eq!(env_or_uint::<u16>("__NONEXISTENT_TEST_VAR__", 42), 42);
        assert_eq!(env_or_uint::<u32>("__NONEXISTENT_TEST_VAR__", 999), 999);
        assert_eq!(
