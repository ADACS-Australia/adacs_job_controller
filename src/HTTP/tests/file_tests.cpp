...
struct FileTestDataFixture : public DatabaseFixture, public HttpServerFixture, public HttpClientFixture
{
    ...
    FileTestDataFixture()
    {
        ...
        jobId1 = database->run(insert_into(jobTable).set(
            jobTable.user       = 1,
        ...
        database->run(insert_into(jobHistoryTable)
                          .set(jobHistoryTable.jobId     = jobId1,
        ...
        // Create the new job object
        jobId2 = database->run(insert_into(jobTable).set(
            jobTable.user       = 1,
            jobTable.parameters = "params1",
            jobTable.cluster = std::static_pointer_cast<HttpServer>(httpServer)->getvJwtSecrets()->at(0).clusters()[0],
            jobTable.bundle  = "whatever",
        ...
        // Create the first state object
        database->run(insert_into(jobHistoryTable)
                          .set(jobHistoryTable.jobId     = jobId2,
                               jobHistoryTable.timestamp = std::chrono::system_clock::now(),
                               jobHistoryTable.what      = SYSTEM_SOURCE,
                               jobHistoryTable.state     = static_cast<uint32_t>(JobStatus::PENDING),
                               jobHistoryTable.details   = "Job submitting"));
        ...
        // Create the new job object
        jobId3 = database->run(insert_into(jobTable).set(
            jobTable.user       = 1,
            jobTable.parameters = "params1",
        ...
        // Create the first state object
        database->run(insert_into(jobHistoryTable)
                          .set(jobHistoryTable.jobId     = jobId3,
                               jobHistoryTable.timestamp = std::chrono::system_clock::now(),
                               jobHistoryTable.what      = SYSTEM_SOURCE,
                               jobHistoryTable.state     = static_cast<uint32_t>(JobStatus::PENDING),
                               jobHistoryTable.details   = "Job submitting"));
...
BOOST_AUTO_TEST_CASE(create_download_unauthorized)
{
    ...
    auto result = httpClient.request("POST",
                                     "/job/apiv1/file/",
                                     "",
                                     {
...
{
    // Test creating a file download for job1 - should be successful because job1 was created by app1 making this
    // request
    setJwtSecret(std::static_pointer_cast<HttpServer>(httpServer)->getvJwtSecrets()->at(0).secret());
...
{
    // Test unauthorized user
    auto result = httpClient.request("PATCH",
                                     "/job/apiv1/file/",
                                     "",
                                     {
                                         {"Authorization", "not_valid"}
...
BOOST_AUTO_TEST_CASE(get_file_list_job1_no_jobid_invalid_payload)
{
    ...
    result = httpClient.request("PATCH",
                                "/job/apiv1/file/",
                                jsonParams.dump(),
                                {
                                    {"Authorization", jwtToken.signature()}
