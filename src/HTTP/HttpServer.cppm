...
public:
    ...
    auto name() const -> const auto & { return nameValue; }

    // The secret (JWT Secret) for this application
    auto secret() const -> const auto & { return secretValue; }

    // The list of other applications that this application has access to (Jobs from these applications)
    auto applications() const -> const auto & { return applicationsValue; }

    // The list of clusters this application can submit to
    auto clusters() const -> const auto & { return clustersValue; }
...
public:
    ...
    auto payload() -> const auto & { return payloadValue; }

    // The JwtSecret that successfully decoded the Authorization header
    auto secret() -> const auto & { return secretValue; }
