...
export void JobApi(const std::string& path, std::shared_ptr<HttpServer> server, std::shared_ptr<IApplication> app)
{
    ...
    server->getServer().resource["^" + path + "$"]["POST"] =
        [clusterManager, server](const std::shared_ptr<HttpServerImpl::Response>& response,
    ...
    server->getServer().resource["^" + path + "$"]["GET"] =
        [server](const std::shared_ptr<HttpServerImpl::Response>& response,
            ...
            // Job Step filtering (Must include a job step id and at least one filter parameter). Job step filtering
            // filters by the provided job step's MOST RECENT state. jobSteps:        csv list of:-
            ...
            try
            {
                ...
                auto startTimeGt =
                    std::chrono::sys_seconds{std::chrono::seconds(getQueryParamAsInt(query_fields, "startTimeGt"))};

                auto startTimeLt =
                    std::chrono::sys_seconds{std::chrono::seconds(getQueryParamAsInt(query_fields, "startTimeLt"))};

                auto endTimeGt =
                    std::chrono::sys_seconds{std::chrono::seconds(getQueryParamAsInt(query_fields, "endTimeGt"))};

                auto endTimeLt =
                    std::chrono::sys_seconds{std::chrono::seconds(getQueryParamAsInt(query_fields, "endTimeLt"))};
    ...
    server->getServer().resource["^" + path + "$"]["PATCH"] =
        [clusterManager, server](const std::shared_ptr<HttpServerImpl::Response>& response,
            ...
            try
            {
                ...
                if (std::find(authResult->secret().clusters().begin(),
                              authResult->secret().clusters().end(),
                              std::string{job->cluster}) == authResult->secret().clusters().end())
                {
                    // Invalid cluster
                    throw std::runtime_error("Application " + authResult->secret().name() +
                                             " does not have access to cluster " + std::string{job->cluster});
                }
    ...
    server->getServer().resource["^" + path + "$"]["DELETE"] =
        [clusterManager, server](const std::shared_ptr<HttpServerImpl::Response>& response,
                                 const std::shared_ptr<HttpServerImpl::Request>& request) {
    ...
    const std::vector<std::string>& applications) -> nlohmann::json
{
    ...
    auto systemStartTimeFilter =
        sqlpp::dynamic_select(*database.getDb())
            ...
            .dynamic_from(
                ...
                jobHistoryTable.join(systemStartTimeFilterAggregate)
                    .on(
    ...
    historyResultsFilter.where.add(jobHistoryTable.jobId.in(
    ...
    historyResultsFilter.where.add(
        ...
        and jobHistoryTable.jobId.in(select(jobHistoryTable.jobId).from(jobHistoryTable).where(endTimeFilter))
...
auto getJobs(const std::vector<uint64_t>& ids) -> nlohmann::json
{
    ...
    auto jobResults =
        database->run(select(all_of(jobTable)).from(jobTable).where(jobTable.id.in(sqlpp::value_list(ids))));
