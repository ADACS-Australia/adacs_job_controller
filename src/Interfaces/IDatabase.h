...
public:
    ...
    virtual auto getClusterJobStatusByJobId(uint64_t jobId, const std::string& cluster) const -> std::vector<uint8_t> = 0;
    virtual auto getClusterJobStatusByJobIdAndWhat(uint64_t jobId, const std::string& what, const std::string& cluster) const -> std::vector<uint8_t> = 0;
    virtual auto saveClusterJobStatus(const std::vector<uint8_t>& statusData, const std::string& cluster) const -> uint64_t = 0;
    ...
    virtual auto createOrUpdateBundleJob(const std::vector<uint8_t>& jobData, const std::string& cluster, const std::string& bundleHash) const -> uint64_t = 0;
    virtual auto getBundleJobById(uint64_t id, const std::string& cluster, const std::string& bundleHash) const -> std::vector<uint8_t> = 0;
