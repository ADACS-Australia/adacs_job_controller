...
async fn test_ltk_authentication_success() {
    ...
    let cluster_arc: Arc<dyn ClusterTrait> = {
        ...
        mock_cluster.expect_name().returning(|| "ltk-test".to_string());
        ...
        mock_cluster.expect_cluster_details().returning(|| test_cluster_config_with_ltk("ltk-test", "test-ltk"));
    ...
    mock_manager.expect_remove_connection().returning(|_, _| Box::pin(async {}));
    mock_manager.expect_report_websocket_error().returning(|_, _| ());
    
    let state = make_test_state(db, mock_manager);
    
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel::<WsOutbound>();
    let result = state.cluster_manager.handle_new_connection(1, tx, "test-ltk").await;
...
async fn test_invalid_ltk_falls_back_to_uuid() {
    ...
    let cluster_arc: Arc<dyn ClusterTrait> = {
        let mut mock_cluster = MockClusterTrait::new();
        mock_cluster.expect_name().returning(|| "test-cluster".to_string());
        mock_cluster.expect_is_online().returning(|| false);
        mock_cluster.expect_set_connection().returning(|_| ());
        mock_cluster.expect_role().returning(|| ClusterRole::Master);
        mock_cluster.expect_role_string().returning(|| "master".to_string());
        mock_cluster.expect_cluster_details().returning(|| test_cluster_config("test-cluster"));
        mock_cluster.expect_send_message().returning(|_| ());
        Arc::new(mock_cluster)
    };
    ...
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel::<WsOutbound>();
    let result = state.cluster_manager.handle_new_connection(1, tx, "test-uuid-12345").await;
...
async fn test_ltk_clusters_skipped_in_reconnect() {
    ...
    assert!(ltk_config.ltk.is_some(), "LTK cluster should have ltk field");
    assert!(ssh_config.ltk.is_none(), "SSH cluster should not have ltk field");
}
