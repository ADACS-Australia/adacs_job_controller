...
async fn test_reconnect_clusters_skips_online() {
    ...
    insert_cluster_uuid(&db, CLUSTER2, &uuid, chrono::Utc::now().naive_utc()).await;
...
async fn test_handle_new_connection_expire_uuids() {
    ...
    insert_cluster_uuid(&db, CLUSTER1, "expired_uuid", expired_ts).await;
    ...
    insert_cluster_uuid(&db, CLUSTER1, "recent_uuid", recent_ts).await;
...
async fn test_handle_new_connection_valid_uuid() {
    ...
    for i in 0..5 {
        ...
        insert_cluster_uuid(&db, CLUSTER2, &last_uuid, chrono::Utc::now().naive_utc()).await;
    }
...
async fn test_handle_new_connection_already_connected_rejected() {
    ...
    insert_cluster_uuid(&db, CLUSTER2, "uuid-first", chrono::Utc::now().naive_utc()).await;
    ...
    insert_cluster_uuid(&db, CLUSTER2, "uuid-second", chrono::Utc::now().naive_utc()).await;
...
async fn test_remove_connection() {
    ...
    for i in 1..=3u64 {
        ...
        insert_cluster_uuid(&db, &name, &uuid, chrono::Utc::now().naive_utc()).await;
