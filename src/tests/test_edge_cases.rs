...
/// Manager with the standard online-cluster + upload-cluster mock setup used by upload tests.
fn manager_with_online_and_upload_clusters<T: ClusterTrait + 'static>(
    cluster_main: &Arc<T>,
    upload_cluster: &Arc<T>,
    fu_for_manager: Arc<FileUploadState>,
...
///
/// # Act
/// Sends a GET request without the `forceDownload` query parameter.
///
/// # Assert
/// Response is 200 OK and `Content-Disposition` contains `inline` and the filename `report.pdf`.
#[tokio::test]
async fn test_download_without_force_download_sets_inline_disposition() {
    let db = setup_test_db().await;
    ...
    assert!(
        ...
        "download without forceDownload should set Content-Disposition: inline; got: {content_disp}"
    );
    assert!(
...
/// Verifies that a download path whose basename contains non-ASCII characters is sanitized to
/// ASCII-only, so the `Content-Disposition` header value stays a valid ASCII quoted-string.
///
/// # Setup
/// Inserts a download record for `/path/to/résumé.txt`; simulates a small 5-byte file stream.
///
/// # Act
/// Sends a GET request for the download.
///
/// # Assert
/// Response is 200 OK and `Content-Disposition` contains the ASCII-only sanitized filename
/// (`r_sum_.txt`) with every non-ASCII byte replaced by `_`.
#[tokio::test]
async fn test_download_sanitizes_non_ascii_filename_in_disposition() {
    let db = setup_test_db().await;

    let uuid_val = "nonascii-dl-uuid".to_string();
    insert_file_download(&db, &uuid_val, "/path/to/résumé.txt").await;
    ...
    assert!(
        content_disp.contains("r_sum_.txt"),
        "Content-Disposition should contain the ASCII-only sanitized filename; got: {content_disp}"
    );
    assert!(
        !content_disp.contains("résumé"),
        "Content-Disposition must not contain non-ASCII bytes from the filename; got: {content_disp}"
    );
}
