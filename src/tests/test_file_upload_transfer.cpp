...
struct FileUploadTransferTestDataFixture : public DatabaseFixture,
                                           public WebSocketClientFixture,
                                           public HttpClientFixture
{
    ...
    FileUploadTransferTestDataFixture() : cluster(clusterManager->getCluster("cluster1"))
    {
                            ...
                            const std::shared_ptr<TestWsClient::InMessage>& in_message)
    {
        ...
        if (msg.getId() == UPLOAD_FILE)
        {
            ...
            websocketFileUploadClient->on_message = [this](const std::shared_ptr<TestWsClient::Connection>& connection,
                                                           const std::shared_ptr<TestWsClient::InMessage>& in_message) {
                auto data = in_message->string();
                auto msg  = Message(std::vector<uint8_t>(data.begin(), data.end()));
...
BOOST_AUTO_TEST_CASE(test_file_upload_transfer)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(fileData.begin(), fileData.end(), originalFileData->begin(), originalFileData->end());
...
BOOST_AUTO_TEST_CASE(test_file_upload_invalid_cluster)
{
    ...
    std::string uploadUrl =
        "/job/apiv1/file/upload/?cluster=invalid_cluster&bundle=whatever&targetPath=/data/myfile.png";
    std::string dummyFileData(1024, 'X');
    auto response = httpClient.request("PUT",
                                       uploadUrl,
                                       dummyFileData,
...
BOOST_AUTO_TEST_CASE(test_file_upload_truncated_data)
{
    ...
    // Upload the file
    std::string uploadUrl =
        "/job/apiv1/file/upload/?jobId=" + std::to_string(jobId) + "&targetPath=/data/truncated_test.bin";
    auto response = httpClient.request("PUT",
                                       uploadUrl,
                                       std::string(originalFileData->begin(), originalFileData->end()),
                                       {
                                           { "Authorization",         jwtToken.signature()},
                                           {  "Content-Type",   "application/octet-stream"},
                                           {"Content-Length", std::to_string(testFileSize)}
    ...
    BOOST_CHECK_EQUAL(response->status_code,
                      "400 Bad Request");  // Should return 400 Bad Request when client sends FILE_UPLOAD_ERROR
...
BOOST_AUTO_TEST_CASE(test_file_upload_with_cluster_bundle_parameters)
{
    ...
    BOOST_CHECK_EQUAL(receivedTargetPath,
                      "/data/cluster_upload.bin");         // Should match the targetPath from URL parameter
...
BOOST_AUTO_TEST_CASE(test_upload_message_format_validation_bundle_based)
{
    ...
    std::string uploadUrl = "/job/apiv1/file/upload/?cluster=" + cluster->getName() +
                            "&bundle=direct_bundle&targetPath=/data/bundle_test.bin";
