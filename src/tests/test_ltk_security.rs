...
use adacs_job_controller::cluster::traits::{ClusterManagerTrait, ClusterTrait, MockClusterManagerTrait, MockClusterTrait, WsOutbound};
...
async fn test_duplicate_ltk_connection_rejected() {
    ...
    let cluster_arc: Arc<dyn ClusterTrait> = {
        ...
        mock_cluster.expect_name().returning(|| "ltk-test-cluster".to_string());
        ...
        mock_cluster.expect_role_string().returning(|| "master".to_string());
        mock_cluster.expect_cluster_details().returning(|| test_cluster_config_with_ltk("ltk-test-cluster", "test-ltk-secret"));
    ...
    mgr.expect_remove_connection().returning(|_, _| Box::pin(async {}));
    ...
    let result1 = state.cluster_manager.handle_new_connection(1, tx1, "test-ltk-secret").await;
    assert!(result1.is_some(), "First LTK connection should succeed");
    
    let (tx2, _rx2) = tokio::sync::mpsc::unbounded_channel::<WsOutbound>();
    let result2 = state.cluster_manager.handle_new_connection(2, tx2, "test-ltk-secret").await;
    assert!(result2.is_none(), "Duplicate LTK connection should be rejected");
}
...
async fn test_rate_limiting_applies() {
    ...
    let cluster_arc: Arc<dyn ClusterTrait> = {
        let mut mock_cluster = MockClusterTrait::new();
        mock_cluster.expect_name().returning(|| "ltk-rate-cluster".to_string());
        mock_cluster.expect_is_online().returning(|| false);
        mock_cluster.expect_set_connection().returning(|_| ());
        mock_cluster.expect_role().returning(|| ClusterRole::Master);
        mock_cluster.expect_role_string().returning(|| "master".to_string());
        mock_cluster.expect_cluster_details().returning(|| test_cluster_config_with_ltk("ltk-rate-cluster", "rate-limit-ltk"));
        mock_cluster.expect_send_message().returning(|_| ());
        Arc::new(mock_cluster)
    };
    ...
    let result = state.cluster_manager.handle_new_connection(1, tx, "rate-limit-ltk").await;
...
async fn test_rate_limiting_disabled_in_test() {
    ...
    let cluster_arc: Arc<dyn ClusterTrait> = {
        let mut mock_cluster = MockClusterTrait::new();
        mock_cluster.expect_name().returning(|| "ltk-no-delay-cluster".to_string());
        mock_cluster.expect_is_online().returning(|| false);
        mock_cluster.expect_set_connection().returning(|_| ());
        mock_cluster.expect_role().returning(|| ClusterRole::Master);
        mock_cluster.expect_role_string().returning(|| "master".to_string());
        mock_cluster.expect_cluster_details().returning(|| test_cluster_config_with_ltk("ltk-no-delay-cluster", "no-delay-ltk"));
        mock_cluster.expect_send_message().returning(|_| ());
        Arc::new(mock_cluster)
    };
    ...
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel::<WsOutbound>();
    let result = state.cluster_manager.handle_new_connection(1, tx, "no-delay-ltk").await;
