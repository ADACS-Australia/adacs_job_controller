...
async fn assert_delete_returns_bad_request_for_status(status: JobStatus) {
    let db = setup_test_db().await;
    let job_id = insert_test_job_with_state(&db, status).await;
...
/// Tests that a blank jobSteps filter (present but empty after trimming) returns an empty
/// list rather than silently dropping the filter and returning all jobs.
///
/// # Setup
/// Inserts one job (job1=Pending).
///
/// # Act
/// Sends GET /job/apiv1/job/?jobSteps=%20.
///
/// # Assert
/// Verifies an empty array is returned (the blank filter must not widen results).
#[tokio::test]
async fn test_get_jobs_blank_job_steps_returns_empty() {
    let db = setup_test_db().await;
    let job1 = insert_test_job(&db, "ozstar", "b1", "testapp").await;
    insert_job_history(&db, job1, JobStatus::Pending as i32, "system").await;

    let body = get_jobs_with_query(db, "?jobSteps=%20").await;
    let jobs = body.as_array().unwrap();
