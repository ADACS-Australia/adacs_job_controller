...
struct JobTestDataFixture : public DatabaseFixture, public HttpServerFixture, public HttpClientFixture
{
    ...
    JobTestDataFixture() : cluster(std::static_pointer_cast<ClusterManager>(clusterManager)->getvClusters()->at(1))
    {
...
BOOST_AUTO_TEST_CASE(test_POST_new_job)
{
    ...
    auto response = client.request("POST",
                                   "/job/apiv1/job/",
                                   "",
                                   {
                                       {"Authorization", "not_valid"}
    });
    ...
    // Check that the job that was created is correct
    jobResults = database->run(select(all_of(jobTable)).from(jobTable).where(jobTable.id == jobId));
...
BOOST_AUTO_TEST_CASE(test_GET_get_jobs)
{
    ...
    auto response = client.request("GET",
                                   ...
                                   {{"Authorization", "not-real"}});
    BOOST_CHECK_EQUAL(std::stoi(response->status_code),
                      static_cast<int>(SimpleWeb::StatusCode::client_error_forbidden));
...
{
    // Test unauthorized user
    auto response = client.request("PATCH",
                                   "/job/apiv1/job/",
                                   {
    },
                                   {{"Authorization", "not-real"}});
    ...
    auto jobId = database->run(insert_into(jobTable).set(
        jobTable.user        = 1,
        jobTable.parameters  = "params1",
    ...
    BOOST_CHECK_EQUAL(std::stoi(response->status_code), static_cast<int>(SimpleWeb::StatusCode::success_ok));
    BOOST_CHECK_EQUAL(response->header.find("Content-Type")->second, "application/json");
    BOOST_CHECK_MESSAGE((*cluster->getqueue())[Message::Priority::Medium].find(source) ==
    ...
    auto validStates =
        std::vector<JobStatus>{JobStatus::SUBMITTING, JobStatus::SUBMITTED, JobStatus::QUEUED, JobStatus::RUNNING};
...
BOOST_AUTO_TEST_CASE(test_DELETE_delete_job)
{
    ...
    auto validStates = std::vector<JobStatus>{JobStatus::CANCELLED,
                                              JobStatus::ERROR,
                                              JobStatus::WALL_TIME_EXCEEDED,
                                              JobStatus::OUT_OF_MEMORY,
                                              JobStatus::COMPLETED};
